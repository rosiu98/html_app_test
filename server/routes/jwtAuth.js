...
const { s3Uploadv2Picture } = require('../s3Service');
...
router.post('/register', upload.single('image'), validInfo, async (req, res) => {
    try {
        ...
        const newUser = await db.query("INSERT INTO users (user_name, user_email, user_token) VALUES ($1, $2, $3) RETURNING id, user_name, user_email", [name, email , bcryptPassword])

        // // //5. Upload and Create url from AWS

        const {Location} = await s3Uploadv2Picture(image , `ProfilePicture${newUser.rows[0].id}`)

        const updatedUser = await db.query("UPDATE users SET user_image = $1 WHERE id = $2 returning id, user_name, user_email, user_image", [Location, newUser.rows[0].id])
...
router.post('/login', validInfo, async (req, res) => {
    try {
        ...
        const validPassword = await bcrypt.compare(password, user.rows[0].user_token);
...
router.get("/profile/:id", async (req, res) => {
    try {
        ...
        const {rows} = await db.query("SELECT * FROM email_table WHERE user_id = $1 ORDER BY id DESC;", [id])
